  ...
  <div class="content-panel">
    ...
    <div class="word-count" v-if="currentFile">
      ...
      <span class="save-state" :class="store.saveState">
        {{ store.saveState === 'saving' ? '保存中...' : '已保存' }}
      </span>
    </div>
  </div>
</template>
...
import {
  ...
  exportFileAsText, downloadFile, pushSnapshot, undo, redo
...
function renderMd(text) {
  ...
  // Don't apply highlightText to rendered HTML (would match data-line etc.)
  // Search highlight is shown in file list and block items instead
...
// Take snapshot when user finishes editing a block (textarea loses focus)
watch(editingBlockId, (newVal, oldVal) => {
...
.save-state { font-size: 10px; color: var(--text-muted); }
