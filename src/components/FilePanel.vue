...
const filteredFiles = computed(() => {
  ...
  // File scope: filter by file name only
  if (store.searchScope === 'file') {
    return files.filter(f => f.name.toLowerCase().includes(q))
  }

  // Category / Global scope: search file name + content (works for all languages, including Chinese)
  return files.filter(f => {
    if (f.name.toLowerCase().includes(q)) return true
    if (f.type === 'markdown' || f.type === 'code') {
