...
import { store, addCategory, selectCategory, addFile, selectFile, showToast, tryLoadFromFile } from './stores/useStore.js'
...
onMounted(async () => {
  ...
  try {
    ...
    getCurrentWindow().setTitle(`Quill v${version}`)
  } catch (_) { /* non-Tauri env */ }

  // Try loading from Tauri file storage (overrides localStorage if available)
