...
📧 **[Tmzzy424@gmail.com](mailto:Tmzzy424@gmail.com)**

如果觉得 Quill 轻量笔记不错，对您有帮助的话，欢迎前往 GitHub 为源码点个 **Star ⭐**，您的支持是我持续更新的最大动力！
🔗 **[GitHub 源码地址](https://github.com/Mingjw1/quill)**
...
// Tauri environment: try loading from app data directory file (overrides localStorage)
export async function tryLoadFromFile() {
  if (typeof window !== 'undefined' && (window.__TAURI_INTERNALS__ || window.__TAURI__)) {
    try {
      const { readTextFile } = await import('@tauri-apps/plugin-fs')
      const { appDataDir } = await import('@tauri-apps/api/path')
      const dir = await appDataDir()
      const fileContent = await readTextFile(dir + 'quill-data.json')
      if (fileContent) {
    ...
    } catch (e) { /* file not found or read failed */ }
  }
  return false
}
...
const miniSearch = new MiniSearch({
  ...
  storeFields: ['name', 'tag', 'catId'],
  idField: 'id',
  searchOptions: { prefix: true, fuzzy: 0.2 }
})

function extractFileText(file) {
  if (file.type === 'markdown' || file.type === 'code' || file.type === 'html') {
    return file.content || ''
  }
  return (file.blocks || []).flatMap(b => (b.items || []).map(i => [i.label, i.text].filter(Boolean).join(' '))).join(' ')
}

export function buildSearchIndex() {
  const docs = []
  for (const catId in store.files) {
    for (const file of store.files[catId]) {
      docs.push({
        id: file.id,
        name: file.name,
        tag: file.tag || '',
        content: extractFileText(file),
        catId
      })
    }
  }
...
export function searchFileIds(query) {
  if (!query || !query.trim()) return null
  const results = miniSearch.search(query)
  return new Set(results.map(r => r.id))
}

// Rebuild index after any structural mutation
function rebuildSearchIndex() {
  // Defer to next microtask to let the reactive update settle
  Promise.resolve().then(() => buildSearchIndex())
}
...
export function pushSnapshot() {
  ...
  store._history.push(snap)
  if (store._history.length > MAX_HISTORY) store._history.shift()
  store._future = []
}

export function undo() {
  if (store._history.length === 0) return
  const current = JSON.parse(JSON.stringify({
    categories: store.categories,
    files: store.files,
    passwords: store.passwords
  }))
  store._future.push(current)
  const prev = store._history.pop()
  store.categories = prev.categories
  store.files = prev.files
  store.passwords = prev.passwords
}
...
export function redo() {
  ...
  store._history.push(current)
  const next = store._future.pop()
  store.categories = next.categories
  store.files = next.files
  store.passwords = next.passwords
}

// Auto-save to localStorage and Tauri file
...
watch(() => {
  ...
  saveTimeout = setTimeout(() => {
    ...
    // Tauri: also write to app data directory
    if (typeof window !== 'undefined' && (window.__TAURI_INTERNALS__ || window.__TAURI__)) {
      import('@tauri-apps/plugin-fs').then(({ writeTextFile }) =>
        import('@tauri-apps/api/path').then(({ appDataDir }) =>
          appDataDir().then(dir =>
            writeTextFile(dir + 'quill-data.json', JSON.stringify(val))
          )
        )
      ).catch(e => console.warn('file persist failed:', e))
    }
    store.saveState = 'saved'
    // Rebuild search index after save settles
    buildSearchIndex()
  }, 1000)
}, { deep: true })

// Build initial search index after module load
Promise.resolve().then(() => buildSearchIndex())
